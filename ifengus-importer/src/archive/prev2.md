...
// Removes video markup before the body reaches the DOM parser. A self-closing
// <video ... /> would otherwise swallow every following sibling into one node.
// Videos are collected by VIDEO_RE and appended separately.
const VIDEO_STRIP_RE = /<video\b[^>]*\/>|<video\b[\s\S]*?<\/video>/gi;
...
class Importer {
  ...
  private async buildBlocks(a: Scraped): Promise<BlockDTO[]> {
    const seeds = parseHtmlToBlockSeeds(
      a.bodyHtml.replace(VIDEO_STRIP_RE, ''),
      extensions, `${ORIGIN}/a/${a.aid}.html`, DETECT_CAPTIONS
    );
